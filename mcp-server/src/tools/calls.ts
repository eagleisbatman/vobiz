...
export function registerCallTools(server: McpServer, client: VobizClient) {
  ...
  server.registerTool(
    "vobiz_voice_make_call",
    {
      ...
      inputSchema: {
        from: z.string().describe("Caller ID in E.164 format (e.g. 914155551234)"),
  ...
  server.registerTool(
    "vobiz_voice_transfer_call",
    {
      ...
      inputSchema: {
        call_uuid: z.string().describe("UUID of the active call to transfer"),
    ...
    async ({ call_uuid, ...body }) => {
      const result = await client.post(`/Account/${id}/Call/${call_uuid}/`, body);
      return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    "vobiz_voice_hangup_call",
    {
      ...
      inputSchema: {
        call_uuid: z.string().describe("UUID of the call to hang up"),
      },
    },
    async ({ call_uuid }) => {
      const result = await client.delete(`/Account/${id}/Call/${call_uuid}/`);
      return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    "vobiz_voice_get_live_call",
    {
      ...
      inputSchema: {
        call_uuid: z.string().describe("UUID of the live call"),
      },
    },
    async ({ call_uuid }) => {
      const result = await client.get(`/account/${id}/call/${call_uuid}/`, { status: "live" });
      return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
    }
  );
